...
(deftest interval-query-check
  (let [x (interval-map {[1 3] :x1
    ...
    (testing "pointwise query"   ;; note: order returned determined by interval start
      ...
      (testing "keyval lookup (find)"
        (is (= [1 [[[0 5] :x4] [[1 3] :x1]]] (find x 1)))
        (is (= [2 [[[0 5] :x4] [[1 3] :x1]]] (find x 2)))
        (is (= [3 [[[0 5] :x4] [[1 3] :x1] [[3 9] :x7]]] (find x 3)))
        (is (= [4 [[[0 5] :x4] [[3 9] :x7] [[4 5] :x8] [[4 7] :x2]]] (find x 4)))
        (is (empty? (find x 9.00000001)))))
